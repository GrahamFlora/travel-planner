...
const parseTimeForSorting = (timeStr) => {
    ...
    const firstMatch = timeStr.match(/(\d{1,2}):(\d{2})\s*(AM|PM)?/i); 
    if (!firstMatch) return 9999; 
    
    let hours = parseInt(firstMatch[1]); 
    let minutes = parseInt(firstMatch[2]); 
    let period = firstMatch[3] ? firstMatch[3].toUpperCase() : null;
    
    // Smart lookahead for time ranges like "3:15 - 4:15 PM" or "10:00 - 12:00 PM"
    if (!period) {
        const periodMatch = timeStr.match(/(AM|PM)/i);
        if (periodMatch) {
            const fallbackPeriod = periodMatch[1].toUpperCase();
            const secondMatch = timeStr.match(/-\s*(\d{1,2}):(\d{2})/);
            
            if (secondMatch) {
                let secondHour = parseInt(secondMatch[1]);
                // Map 12 to 0 to correctly identify noon/midnight crossovers (e.g., 11 AM to 1 PM)
                let h1 = hours === 12 ? 0 : hours;
                let h2 = secondHour === 12 ? 0 : secondHour;
                
                if (h1 > h2) {
                    period = fallbackPeriod === 'PM' ? 'AM' : 'PM';
                } else {
    ...
    return (
        <div className="min-h-screen transition-colors duration-500 bg-zinc-100 dark:bg-slate-950 text-zinc-900 dark:text-slate-100 font-sans pb-32 overflow-x-hidden print:bg-white print:text-black print:pb-0">
            ...
            <div className="hidden print:block w-full bg-white text-black font-sans">
                {/* CSS to hide browser URL/Time and enforce clean printing margins */}
                ...
                <div className="border-b-2 border-slate-800 pb-6 mb-8 text-center">
                    <h1 className="text-4xl font-black text-slate-900 uppercase tracking-widest mb-2">{trip.title}</h1>
                    <p className="text-xl font-bold text-slate-700 tracking-wider">OFFICIAL ITINERARY</p>
                    ...
                    <p className="text-sm text-slate-600 mt-4">
                        <strong>Travelers:</strong> {user.email} (Primary)
                        {trip.companions && trip.companions.length > 0 && `, ${trip.companions.map(c => typeof c === 'string' ? c : c.name).join(', ')}`}
                    </p>
                </div>

                {trip.days.map((day, dIdx) => (
                    <div key={day.id} className={`mb-12 ${dIdx > 0 ? 'print-page-break' : ''}`}>
                        
                        {/* Day Header */}
                        <div className="bg-slate-100 p-4 rounded-lg mb-6 border-l-4 border-slate-800 print-no-break">
                            <h2 className="text-2xl font-black text-slate-900 uppercase tracking-wide">Day {dIdx + 1}: {day.title}</h2>
                            <p className="font-bold text-slate-600 mt-1">{new Date(day.date).toLocaleDateString(undefined, { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' })}</p>
                            {day.summary && <p className="mt-3 text-slate-700 text-sm font-medium">{day.summary}</p>}
                        </div>

                        {day.activities.length === 0 ? (
                            <p className="text-slate-500 italic text-center py-8">No scheduled activities for this day.</p>
                        ) : (
                            <table className="w-full text-left border-collapse">
                                <thead>
                                    <tr className="border-b-2 border-slate-800">
                                        <th className="py-3 px-2 w-36 text-sm font-black uppercase tracking-wider text-slate-800">Time</th>
                                        <th className="py-3 px-2 text-sm font-black uppercase tracking-wider text-slate-800">Activity Details</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {day.activities.map((act, aIdx) => (
                                        <tr key={act.id} className="border-b border-slate-200 print-no-break">
                                            <td className="py-4 px-2 align-top w-36">
                                                <span className="font-bold text-slate-900 whitespace-nowrap">{act.time}</span>
                                            </td>
                                            <td className="py-4 px-2 align-top">
                                                <h3 className="font-bold text-lg text-slate-900 leading-tight">{act.title}</h3>
                                                {act.desc && act.desc !== act.time && <p className="text-sm text-slate-600 mt-1 font-medium">{act.desc}</p>}
                                                {act.details && (
                                                    <p className="text-sm text-slate-700 whitespace-pre-wrap mt-2 bg-slate-50 p-3 rounded-lg border border-slate-100">{act.details}</p>
                                                )}
                                            </td>
                                        </tr>
                                    ))}
